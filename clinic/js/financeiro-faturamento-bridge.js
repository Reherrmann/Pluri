// Mentalita — ponte Faturamento → Financeiro
(function(){
  function init(){
    if(!window.PLURI_SUPABASE)return;
    window.MentalitaFinancialBridge={
      async createReceivableFromBilling({billingId,description,amount,dueAt,source='Faturamento',patientId=null}={}){
        ...
        const existing=await sb.from('mentalita_financial_receivables').select('id').eq('clinic_id',clinicId).eq('source_type','faturamento').eq('source_id',billingId).maybeSingle();
        ...
        const {data,error}=await sb.from('mentalita_financial_receivables').insert(payload).select().single();
      ...
      async registerBillingReceipt({billingId,receivedAt=null}={}){
        ...
        const {data,error}=await sb.from('mentalita_financial_receivables').update({status:'received',received_at:receivedAt||new Date().toISOString().slice(0,10)}).eq('clinic_id',clinicId).eq('source_type','faturamento').eq('source_id',billingId).select().maybeSingle();
        if(error)throw error;return data;
      }
    };
  }
