...
function installConveniosModule() {
    const install = () => {
        ...
        if (nav && !nav.querySelector('[data-page="convenios"]')) {
            ...
            link.addEventListener('click', e => { e.preventDefault(); if (typeof navigateTo === 'function') navigateTo('convenios'); });
        ...
        if (typeof originalRenderPage === 'function' && !window.__pluriConveniosRenderWrapped) {
            ...
            window.renderPage = function () {
                if (window.state?.currentPage !== 'convenios') return originalRenderPage.apply(this, arguments);
...
(function loadAgendamentosPacienteModule() {
    const script = document.createElement('script');
    script.src = 'js/agendamentos-paciente.js?v=1';
    script.onload = () => console.log('✅ Módulo de agendamentos do paciente carregado.');
    script.onerror = () => console.error('❌ Não foi possível carregar agendamentos-paciente.js');
    document.head.appendChild(script);
})();
...
(function installAgendaMonthLayout() {
    ...
    const install = () => {
        ...
        style.id = styleId;
        style.textContent = `
            .agenda-month-grid { align-items: stretch; }
            .agenda-month-cell { min-height: 56px !important; height: auto !important; padding: 7px !important; overflow: hidden; }
            .agenda-month-cell:has(.agenda-month-event) { min-height: 82px !important; }
            .agenda-month-cell:has(.agenda-month-more) { min-height: 96px !important; }
            .agenda-month-cell.empty { min-height: 56px !important; }
            .agenda-month-daynum { margin-bottom: 5px !important; line-height: 1.1; }
            .agenda-month-events { gap: 3px !important; }
            .agenda-month-event { min-height: 20px; line-height: 20px; padding: 0 4px !important; overflow: hidden; }
            .agenda-month-event-name { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        `;
...
(function loadAgendaMonthCompactCSS() {
    ...
    if (document.querySelector('link[href^="css/agenda-month-compact.css"]')) return;
