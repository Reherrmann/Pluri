...
if (!content) return;
content.innerHTML = <h3 style="margin-bottom:16px;">Novo paciente</h3> <div class="form-group"><label>Nome</label><input type="text" id="newPatientName"></div> <div class="form-group"><label>Telefone</label><input type="text" id="newPatientPhone"></div> <div class="form-group"><label>E-mail</label><input type="email" id="newPatientEmail"></div> <div class="form-group"><label>Observações</label><textarea id="newPatientNotes" rows="3"></textarea></div> <div style="margin-top:16px;display:flex;gap:8px;"> <button class="btn btn-outline btn-sm" id="cancelNewPatient">Cancelar</button> <button class="btn btn-primary btn-sm" id="saveNewPatient">Criar paciente</button> </div> <p style="font-size:11px;color:var(--text-secondary);margin-top:8px;">O paciente será salvo direto no Google Sheets.</p>;
...
const content = getEl('slideContent');
if (!content) return;
content.innerHTML = <h3 style="margin-bottom:16px;">Editar paciente</h3> <div class="form-group"><label>Nome</label><input type="text" id="editPatientName" value="${p.name}"></div> <div class="form-group"><label>Telefone</label><input type="text" id="editPatientPhone" value="${p.phone}"></div> <div class="form-group"><label>E-mail</label><input type="email" id="editPatientEmail" value="${p.email || ''}"></div> <div class="form-group"><label>Observações</label><textarea id="editPatientNotes" rows="3">${p.notes || ''}</textarea></div> <div style="margin-top:16px;display:flex;gap:8px;"> <button class="btn btn-outline btn-sm" id="cancelPatientEdit">Cancelar</button> <button class="btn btn-primary btn-sm" id="savePatientEdit">Salvar alterações</button> </div> <p style="font-size:11px;color:var(--text-secondary);margin-top:8px;">As alterações serão sincronizadas com o Google Sheets.</p>;
