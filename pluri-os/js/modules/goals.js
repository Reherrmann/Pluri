 ...
 * Integrado com Google Sheets (aba "Metas")
 */
const Goals = (() => {
  const SHEET_NAME = 'Metas';

  /**
   * Sincroniza metas da planilha com localStorage
   */
  ...
  function renderGoalCard(goal) {
    ...
    const periodLabels = {
      daily: 'Diária', weekly: 'Semanal', monthly: 'Mensal', quarterly: 'Trimestral', annual: 'Anual'
    };
  ...
  async function saveGoal() {
    ...
    Components.showToast(success ? 'Meta salva na planilha!' : 'Salva localmente, mas falha ao enviar para planilha.', success ? 'success' : 'warning');
  ...
  function updateProgress(id) {
    ...
    Components.openModal({
      ...
      bodyHTML: `
        <p>Meta: <strong>${goal.description}</strong></p>
        <div class="form-group">
          <label class="form-label">Valor Atual</label>
          <input type="number" id="update-current" class="form-input" step="0.01" value="${goal.current}">
        </div>
      `,
      footerHTML: `
        <button class="btn-secondary" onclick="Components.closeModal()">Cancelar</button>
        <button class="btn-primary" onclick="Goals.saveProgress('${id}')">Atualizar</button>
      `,
    });
  }
  ...
  async function saveProgress(id) {
    ...
    // Atualiza a planilha (usando update na coluna ID)
    const success = await GoogleSheets.updateCell(SHEET_NAME, 'Atual', newCurrent); // ATENÇÃO: precisa ajustar updateCell para buscar por ID
  ...
  async function deleteGoal(id) {
    ...
    Components.confirmDialog({
      ...
      onConfirm: async () => {
        ...
        // Remove da planilha
        if (goal.source === 'planilha' || goal.id) {
          await GoogleSheets.deleteRow(SHEET_NAME, goal.id);
        }
