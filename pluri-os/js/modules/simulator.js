...
const Simulator = (() => {
    function render() {
        ...
        return `
            <div class="fade-in">
                <h3 style="font-weight:600;margin-bottom:20px">Simulador Financeiro</h3>
    ...
    function calculate(inputs) {
        ...
        const receitaMensal = mensalidade * clientes;
        const receitaAnual = receitaMensal * 12 + receitaImplantacao;
        const receita24 = receitaMensal * 24 + receitaImplantacao;
        const receita36 = receitaMensal * 36 + receitaImplantacao;
        const totalComissao = (receitaMensal * contrato + receitaImplantacao) * (comissao / 100) * clientes;
        const lucroEstimado = (receitaMensal * contrato + receitaImplantacao) - totalComissao;
        const roi = receitaImplantacao > 0 ? ((lucroEstimado / (receitaImplantacao * clientes)) * 100).toFixed(0) : 0;
    ...
    function renderResults(r) {
        return `
            <div class="cards-grid" style="grid-template-columns:1fr">
                ...
                ${Components.metricCard({ title: 'Receita Mensal', value: Utils.formatCurrency(r.receitaMensal), icon: 'calendar', color: 'accent' })}
                ${Components.metricCard({ title: 'Receita Anual', value: Utils.formatCurrency(r.receitaAnual), icon: 'trending-up', color: 'success' })}
                ${Components.metricCard({ title: 'Receita 24 Meses', value: Utils.formatCurrency(r.receita24), icon: 'bar-chart', color: 'success' })}
                ${Components.metricCard({ title: 'Receita 36 Meses', value: Utils.formatCurrency(r.receita36), icon: 'bar-chart-3', color: 'success' })}
                ${Components.metricCard({ title: 'Comissão Total', value: Utils.formatCurrency(r.totalComissao), icon: 'users', color: 'warning' })}
                ${Components.metricCard({ title: 'Lucro Estimado', value: Utils.formatCurrency(r.lucroEstimado), icon: 'dollar-sign', color: 'success' })}
