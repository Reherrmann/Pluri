...
(function installMentalitaPatientProntuario() {
  ...
  async function sha256(text){ const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(text)); return Array.from(new Uint8Array(digest)).map(b=>b.toString(16).padStart(2,'0')).join(''); }
  ...
  async function getStaffMap(records){
    const client=getClient(); const ids=[...new Set(records.map(r=>r.professional_id).filter(Boolean))];
  ...
  async function saveRecord(status){
    ...
    const record={clinic_id:clinicId,patient_id:id,professional_id:u?.id||null,created_by:u?.id||null,record_type:type,attended_at:now,created_at:now,status,content:{title,text},title,finalized_at:status==='finalized'?now:null,finalized_by:status==='finalized'?(u?.id||null):null};
    ...
    await audit(data.id,status==='finalized'?'record_finalized':'record_created',{record_type:type,title,status});
  ...
  function pdfPage(title,subtitle,body){
    return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>${esc(title)}</title><style>@page{size:A4;margin:18mm 16mm}*{box-sizing:border-box}body{font-family:Inter,Arial,sans-serif;color:#18232d;margin:0;line-height:1.55;font-size:11.5px}.head{border-bottom:2px solid #183b56;padding-bottom:14px;margin-bottom:22px}.brand{font-size:11px;letter-spacing:.16em;text-transform:uppercase;font-weight:800;color:#183b56}.title{font-size:25px;font-weight:800;margin:5px 0}.sub{color:#647582}.patient{background:#f4f7f9;border:1px solid #dce5eb;border-radius:10px;padding:12px 14px;margin:16px 0}.section{margin:22px 0}.section-title{font-size:13px;text-transform:uppercase;letter-spacing:.08em;font-weight:800;color:#183b56;border-bottom:1px solid #dce5eb;padding-bottom:6px;margin-bottom:12px}.entry{border-left:3px solid #183b56;padding:0 0 16px 14px;margin:0 0 18px}.entry-head{display:flex;justify-content:space-between;gap:12px;font-weight:800}.entry-type{font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:#183b56}.entry-date{font-size:10px;color:#647582}.entry-prof{font-size:10.5px;color:#52636f;margin:4px 0 9px}.content{white-space:pre-wrap}.integrity{margin-top:10px;padding:8px 10px;background:#f6f8fa;border-radius:7px;font-size:9px;color:#667785;word-break:break-all}.footer{margin-top:28px;padding-top:10px;border-top:1px solid #dce5eb;font-size:9px;color:#71818d}.no-print{margin-top:20px;padding:9px 14px}@media print{.no-print{display:none}}</style></head><body>${body}<button class="no-print" onclick="window.print()">Salvar em PDF</button></body></html>`;
  }

  async function exportRecord(record){
    const p=getPatient(),staff=await getStaffMap([record]),s=staff[record.professional_id];
    const name=s?.name||'Profissional não identificado';
    const body=`<div class="head"><div class="brand">Mentalita · Prontuário eletrônico</div><div class="title">Registro clínico</div><div class="sub">Documento individual do atendimento</div></div><div class="patient"><b>Paciente</b><br>${esc(patientName(p))}</div><div class="section"><div class="section-title">${esc(record.record_type||'Registro')} · ${esc(record.content?.title||record.title||'Sem título')}</div><div class="entry"><div class="entry-head"><span>${esc(record.content?.title||record.title||'Registro clínico')}</span><span class="entry-date">${esc(formatDate(record.attended_at||record.created_at))}</span></div><div class="entry-prof">${esc(name)}${s?.role?' · '+esc(s.role):''}</div><div class="content">${esc(record.content?.text||'')}</div><div class="integrity">Status: ${esc(record.status)} · Finalizado: ${esc(formatDate(record.finalized_at))}<br>Hash de integridade: ${esc(record.content_hash||record.integrity_hash||'-')}</div></div></div><div class="footer">Gerado pela Mentalita · O registro persistido no sistema é a fonte oficial. Este PDF é uma exportação.</div>`;
  ...
  async function exportFull(records){
    const p=getPatient(),staff=await getStaffMap(records);const ordered=[...records].sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));
    ...
    const body=`<div class="head"><div class="brand">Mentalita · Prontuário eletrônico</div><div class="title">Prontuário completo</div><div class="sub">Histórico longitudinal do paciente</div></div><div class="patient"><b>Paciente</b><br>${esc(patientName(p))}<br><span class="sub">${ordered.length} registro(s) · período: ${esc(ordered.length?formatDate(ordered[0].created_at):'-')} a ${esc(ordered.length?formatDate(ordered[ordered.length-1].created_at):'-')}</span></div><div class="section"><div class="section-title">Histórico clínico</div>${entries}</div><div class="footer">Gerado pela Mentalita · O conjunto de registros persistidos no sistema é a fonte oficial. Este documento é uma exportação do histórico disponível no momento da geração.</div>`;
  ...
  async function renderTimeline(){
    const box=document.getElementById('mentalitaPatientProntuarioTimeline');if(!box)return;const records=await loadRecords();
    if(!records.length){box.innerHTML='<div style="padding:34px;text-align:center;border:2px dashed var(--border);border-radius:12px"><h3 style="margin:0 0 6px">Nenhum registro clínico</h3><p style="color:var(--text-secondary);margin:0">Os atendimentos e evoluções aparecerão aqui.</p></div>';return;}
    const staff=await getStaffMap(records);await Promise.all(records.map(r=>audit(r.id,'record_viewed',{surface:'timeline'})));
    box.innerHTML=records.map(r=>{const finalized=r.status==='finalized',s=staff[r.professional_id];return `<div style="border:1px solid var(--border);border-radius:14px;padding:16px;margin-bottom:10px;background:var(--surface,#fff)"><div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap"><div><div style="font-weight:800">${esc(r.content?.title||r.title||'Registro clínico')}</div><div style="font-size:11.5px;color:var(--text-secondary);margin-top:3px">${esc(r.record_type||'registro')} · ${esc(formatDate(r.attended_at||r.created_at))}</div><div style="font-size:11px;color:var(--text-secondary);margin-top:3px">${esc(s?.name||'Profissional não identificado')}</div></div><span style="font-size:11px;font-weight:700;padding:5px 9px;border-radius:999px;background:var(--hover-bg)">${finalized?'🔒 Finalizado':'Rascunho'}</span></div><div style="margin-top:12px;white-space:pre-wrap;font-size:13px;line-height:1.65">${esc(r.content?.text||'')}</div><div style="display:flex;justify-content:flex-end;gap:8px;margin-top:12px;flex-wrap:wrap"><button type="button" class="btn mpPdf" data-id="${esc(r.id)}">PDF do registro</button><button type="button" class="btn mpDelete" data-id="${esc(r.id)}">Apagar teste</button></div>${finalized?'<div style="margin-top:9px;font-size:10px;color:var(--text-secondary);word-break:break-all">Hash: '+esc(r.content_hash||r.integrity_hash||'-')+'</div>':''}</div>`;}).join('');
  ...
  function sectionHtml(){
    return `<div class="patient-section"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap"><div><div style="font-size:11px;text-transform:uppercase;letter-spacing:.12em;color:var(--text-secondary);font-weight:800">Histórico clínico</div><h2 style="margin:4px 0">Prontuário eletrônico</h2><p style="margin:0;color:var(--text-secondary);font-size:13px">Um histórico longitudinal com todos os registros deste paciente.</p></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" type="button" id="mentalitaExportFullProntuarioBtn">Exportar prontuário completo</button><button class="btn btn-primary" type="button" id="mentalitaNewProntuarioEntryBtn">+ Novo registro</button></div></div><div class="patient-info-card" style="padding:14px 16px;margin-top:16px"><b style="color:var(--primary)">Registro clínico protegido</b><div style="font-size:11.5px;color:var(--text-secondary);margin-top:4px">Exclusão temporariamente habilitada apenas para testes. Em produção, registros finalizados serão preservados e correções serão novos lançamentos.</div></div><div class="patient-info-card"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px"><div><h3 style="margin:0 0 3px">Linha do tempo clínica</h3><span style="font-size:11.5px;color:var(--text-secondary)">Registros em ordem cronológica</span></div><span style="font-size:11px;font-weight:700;padding:5px 8px;border-radius:999px;background:var(--hover-bg)">Histórico longitudinal</span></div><div id="mentalitaPatientProntuarioTimeline">Carregando...</div></div><div class="patient-info-card"><h3 style="margin-bottom:8px">Integridade e rastreabilidade</h3><div style="font-size:12px;color:var(--text-secondary);line-height:1.6">Cada finalização gera hash de integridade e eventos relevantes são registrados na auditoria. O PDF é uma exportação; o registro persistido no Supabase permanece como fonte oficial.</div></div></div>`;
  }

  function bindSection(){
    const button=document.getElementById('mentalitaNewProntuarioEntryBtn');if(button&&!button.dataset.bound){button.dataset.bound='1';button.addEventListener('click',()=>{document.getElementById('mentalitaProntuarioModal')?.remove();document.body.insertAdjacentHTML('beforeend',modalHtml());document.getElementById('mpClose').addEventListener('click',()=>document.getElementById('mentalitaProntuarioModal')?.remove());document.getElementById('mpDraft').addEventListener('click',()=>saveRecord('draft'));document.getElementById('mpForm').addEventListener('submit',e=>{e.preventDefault();saveRecord('finalized');});});}
    renderTimeline();if(typeof refreshIcons==='function')refreshIcons();
  }
