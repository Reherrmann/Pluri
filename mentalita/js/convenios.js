...
function isCatalogoMentalita(c) {
...
function renderMentalitaConvenios() {
    ...
    const catalogo = rows.filter(isCatalogoMentalita);
    const personalizados = rows.filter(c => !isCatalogoMentalita(c));
    let html = '';
    if (catalogo.length) {
    ...
    if (personalizados.length) {
        html += `<tr class="mentalita-convenios-divider"><td colspan="5">Convênios adicionados pela clínica</td></tr>`;
        html += personalizados.map(c => `<tr data-mc-id="${c.id}" style="cursor:pointer"><td><strong>${escapeHtml(c.name)}</strong>${c.contact ? `<div style="font-size:11px;color:var(--text-secondary)">${escapeHtml(c.contact)}</div>` : ''}</td><td>${escapeHtml(c.ans || '—')}</td><td>${Array.isArray(c.plans) ? c.plans.length : 0}</td><td>${statusBadge(c.status)}</td><td><button type="button" class="mentalita-convenio-delete" data-delete-mc-id="${c.id}" title="Excluir convênio"><i data-lucide="trash-2"></i></button></td></tr>`).join('');
    }
    ...
    body.querySelectorAll('[data-delete-mc-id]').forEach(btn => btn.addEventListener('click', async e => { e.stopPropagation(); await deleteMentalitaConvenio(btn.dataset.deleteMcId); }));
    refreshIcons();
}
...
async function deleteMentalitaConvenio(id) {
    const c = mentalitaConvenios.find(x => x.id === id);
    if (!c || isCatalogoMentalita(c)) return;
    if (!confirm(`Excluir o convênio "${c.name}" cadastrado pela clínica? Essa ação não pode ser desfeita.`)) return;
    const { error } = await window.PLURI_SUPABASE.from('mentalita_convenios').delete().eq('id', id).eq('clinic_id', window.PLURI_CLINIC.id);
    if (error) { console.error('[Mentalita] excluir convênio:', error); alert('Não foi possível excluir o convênio.'); return; }
    await loadMentalitaConvenios();
}
