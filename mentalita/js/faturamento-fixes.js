...
(function(){
  ...
  function fixTabs(){
    document.querySelectorAll('.billing-tab').forEach(btn=>btn.classList.remove('btn-primary'));
    const active=document.querySelector('.billing-tab.active');
    if(active)document.querySelectorAll('.billing-tab').forEach(x=>{if(x!==active)x.classList.remove('active','btn-primary');});
  }
  function run(){fixTabs();fixDrafts();}
  document.addEventListener('click',e=>{if(e.target.closest?.('.billing-tab'))setTimeout(()=>{fixTabs();fixDrafts();},30);});
