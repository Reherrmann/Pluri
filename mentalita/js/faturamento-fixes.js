...
(function(){
  ...
  async function loadGuides(){
    ...
    const q=await c.from('mentalita_patient_tiss_guides').select('id,patient_id,convenio_id,guide_number,execution_date,total_amount,status,created_at').eq('clinic_id',C()).in('status',['draft','ready']).order('created_at',{ascending:false});
    if(q.error){console.error('[Mentalita Faturamento]',q.error);return{drafts:[],ready:[]};}
    const rows=q.data||[];const ids=[...new Set(rows.map(x=>x.patient_id).filter(Boolean))];let names={};
  ...
  async function renderGuides(host){
    ...
    host.innerHTML=css+`<div class="billing-clean"><div class="billing-summary"><div class="billing-summary-card"><small>Prontas para faturamento</small><strong>${ready.length}</strong></div><div class="billing-summary-card"><small>Em preparação</small><strong>${drafts.length}</strong></div><div class="billing-summary-card"><small>Total de guias</small><strong>${ready.length+drafts.length}</strong></div></div><div class="billing-section"><div class="billing-section-head"><div><h3>Prontas para faturamento</h3><p>Selecione as guias que devem entrar no mesmo lote.</p></div>${ready.length?'<button class="btn billing-primary billing-create-batch" type="button" disabled>Criar lote</button>':''}</div><div class="billing-list">${ready.length?ready.map(g=>guideRow(g,true)).join(''):'<div class="billing-empty-clean">Nenhuma guia pronta para faturamento.</div>'}</div></div><div class="billing-section"><div class="billing-section-head"><div><h3>Em preparação</h3><p>Guias criadas, mas ainda não liberadas para faturamento.</p></div></div><div class="billing-list">${drafts.length?drafts.map(g=>guideRow(g,false)).join(''):'<div class="billing-empty-clean">Nenhum rascunho pendente.</div>'}</div></div></div>`;
    const checks=[...host.querySelectorAll('.billing-guide-check')];const createBtn=host.querySelector('.billing-create-batch');const syncSelection=()=>{if(createBtn)createBtn.disabled=!checks.some(x=>x.checked);};checks.forEach(x=>x.addEventListener('change',syncSelection));
  ...
  async function createBatch(host){
    ...
    const{error:aError}=await c.from('mentalita_patient_tiss_audit').insert(auditRows);if(aError)console.warn('[Mentalita Faturamento] Auditoria do lote:',aError);
    alert(`Lote ${batchNumber} criado com ${guides.length} guia${guides.length===1?'':'s'}.`);lastTab='';const lotesTab=document.querySelector('.billing-tab[data-billing-tab="lotes"]');if(lotesTab){lotesTab.click();return;}await renderGuides(host);
  }
  async function renderBatches(host){
    const c=S();if(!c||!C()){host.innerHTML=css+'<div class="billing-section">Não foi possível carregar os lotes.</div>';return;}
    ...
    const rows=data||[];const convenioIds=[...new Set(rows.map(x=>x.convenio_id).filter(Boolean))];let convenios={};if(convenioIds.length){const r=await c.from('mentalita_convenios').select('id,name').in('id',convenioIds);convenios=Object.fromEntries((r.data||[]).map(x=>[x.id,x.name]));}
    host.innerHTML=css+`<div class="billing-clean"><div class="billing-section"><div class="billing-section-head"><div><h3>Lotes</h3><p>Guias agrupadas por convênio e competência, prontas para o próximo passo do faturamento.</p></div></div><div class="billing-list">${rows.length?rows.map(b=>`<div class="billing-row-clean"><div class="billing-main"><div><div class="billing-batch-number">${esc(b.batch_number)}</div><div class="billing-note-clean">${esc(convenios[b.convenio_id]||'Convênio')} · competência ${esc(b.competence_month||'—')} · ${b.guide_count} guia${b.guide_count===1?'':'s'} · ${money(b.total_amount)}</div></div></div><div class="billing-actions"><span class="billing-status">${esc(b.status==='draft'?'Rascunho':b.status)}</span></div></div>`).join(''):'<div class="billing-empty-clean">Nenhum lote criado ainda.</div>'}</div></div></div>`;
  }
  ...
  async function render(){const host=document.getElementById('billingWorkspace');if(!host)return;const active=tabs();if(active===lastTab&&host.querySelector('.billing-clean'))return;lastTab=active;host.dataset.billingRendered='1';if(active==='guias')return renderGuides(host);if(active==='lotes')return renderBatches(host);if(active==='glosas')return renderPlaceholder(host,'Glosas','Registre e acompanhe recusas e pendências dos convênios.');if(active==='recebimentos')return renderPlaceholder(host,'Recebimentos','Acompanhe os pagamentos recebidos dos convênios.');renderOverview(host);}
  ...
  const obs=new MutationObserver(()=>{const host=document.getElementById('billingWorkspace');if(host&&!host.querySelector('.billing-clean')){lastTab='';run();}});obs.observe(document.body,{childList:true,subtree:true});setTimeout(()=>{lastTab='';run();},300);window.mentalitaFaturamentoFix=run;
})();
