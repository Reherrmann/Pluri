...
class PluriAPI {
  ...
  mapPatient(p){ return {
    _row:p.id,id:p.id,name:p.name||'',phone:p.phone||'',email:p.email||'',created:this.formatDate(p.created_at),
    lastVisit:this.formatDate(p.last_visit),nextAppt:this.formatDate(p.next_appointment),status:p.status||'Ativo',notes:p.notes||'',
    birthDate:p.birth_date||p.birthDate||'',age:p.age||'',gender:p.gender||'',maritalStatus:p.marital_status||p.maritalStatus||'',mobile:p.mobile||'',
    sendReminders:p.send_reminders||p.sendReminders||'',zipCode:p.zip_code||p.zipCode||'',address:p.address||'',number:p.address_number||p.number||'',
    complement:p.complement||'',neighborhood:p.neighborhood||'',city:p.city||'',state:p.state||'',cpf:p.cpf||'',rg:p.rg||'',profession:p.profession||'',
    motherName:p.mother_name||p.motherName||'',fatherName:p.father_name||p.fatherName||'',familyContactName:p.family_contact_name||p.familyContactName||'',
    familyContactRelationship:p.family_contact_relationship||p.familyContactRelationship||'',familyContactPhone:p.family_contact_phone||p.familyContactPhone||''
  }; }
  ...
  async updateStaff(id,s){return this.run('Salvando paciente...',()=>this.client.from('mentalita_staff').update({name:s.name,role:s.role||'',email:s.email||'',phone:s.phone||'',status:s.status||'Ativo'}).eq('id',id).eq('clinic_id',this.clinicId)).then(x=>x?{success:true}:{success:false,error:'Não foi possível atualizar o profissional.'});}
